...
it('returns null when provider is not found', function() {
    $providerManager = new ProviderManager();

    expect($providerManager->findProvider('nonexistent'))->toBeNull();
});

it('returns correct provider class name', function() {
    $providerManager = new ProviderManager();
...
it('returns empty list when no providers are registered', function() {
...
it('returns correct list of provider links', function() {
...
it('returns empty list for disabled provider links', function() {
    $provider = Mockery::mock(BaseProvider::class)->makePartial();
...
it('registers a single provider correctly', function() {
    $providerManager = new ProviderManager();
...
it('registers a single provider correctly when code is missing', function() {
...
it('registers callback correctly', function() {
    $providerManager = new ProviderManager();
...
it('creates provider instance correctly', function() {
    $providerManager = new ProviderManager();
    $providerInfo = ['code' => 'test'];
...
it('throws exception when provider class does not exist', function() {
    $providerManager = new ProviderManager();
    expect(fn() => $providerManager->makeProvider('NonExistentClass'))->toThrow(SystemException::class);
});

it('adds callback to resolveUserTypeCallbacks array', function() {
    $providerManager = new ProviderManager();
    $callback = function() {
        return 'custom_user_type';
    };
...
it('executes resolveUserType callback correctly', function() {
    $providerManager = new ProviderManager();
    $callback = function() {
        return 'custom_user_type';
    };
...
it('returns default user type when no callback is provided', function() {
    $providerManager = new ProviderManager();
    $reflection = new ReflectionClass($providerManager);
    $method = $reflection->getMethod('resolveUserTypeCallback');
    $method->setAccessible(true);
...
it('runs entry point and redirects to provider', function() {
    $provider = Mockery::mock(BaseProvider::class)->makePartial();
...
it('handles provider callback, creates user and returns redirect response', function() {
    Event::fake();
...
it('handles provider callback, retrieves user and returns redirect response', function() {
    Event::fake();
...
it('throws exception provider class handleProviderCallback fails', function() {
    Event::fake();
...
it('throws exception when unknown socialite provider is used', function() {
    $providerManager = new ProviderManager();
    $providerManager->registerProvider(BaseProvider::class, ['code' => 'test']);
...
it('bails when provider user is not in session', function() {
    $providerManager = new ProviderManager();
...
it('bails when provider user is not found', function() {
    $providerManager = new ProviderManager();
...
it('bails when completeCallback event halts flow', function() {
    Event::listen('igniter.socialite.completeCallback', function() {
        return true;
    });
...
it('bails when beforeLogin event halts flow', function() {
    Event::listen('igniter.socialite.beforeLogin', function() {
